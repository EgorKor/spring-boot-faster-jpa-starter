...
import ru.samgtu.packages.webutils.queryparam.filterInternal.FilterToken;
import ru.samgtu.packages.webutils.queryparam.filterInternal.FilterTokenType;
import ru.samgtu.packages.webutils.queryparam.filterInternal.FilterTokenizer;
import ru.samgtu.packages.webutils.queryparam.filterInternal.FilterValidationResult;
...
class FilterTokenizerTest {
    ...
    @Test
    @DisplayName("Должен проверять наличие FROM в SUBSTRING")
    void shouldValidateSubstringFromKeyword() {
        // Given
        String expression = "substring(activityKind.description 1 10)";
        ...
        assertFalse(result.isValid());
        assertThat(result.errors()).anyMatch(error ->
                error.contains("SUBSTRING function requires 'FROM' keyword"));
    }
    ...
    void shouldFindInvalidIdentifiers() {
        ...
        assertThat(result.errors()).anyMatch(error ->
                error.contains("Unknown token 'activityKind..area.code' at position 0"));
    }
    ...
    private static Stream<Arguments> provideValidExpressions() {
        return Stream.of(
                ...
                Arguments.of("substring(activityKind.description from 1 for 10)", "SUBSTRING с FROM и FOR"),
                Arguments.of("upper(activityKind.name)", "Простая функция UPPER"),
                Arguments.of("trim(activityKind.code)", "Функция TRIM"),
                Arguments.of("activityKind.area.code = 'TEST'", "Простое сравнение"),
                Arguments.of("length(activityKind.name) > 5", "Функция LENGTH с оператором")
        );
    }
    ...
    private static Stream<Arguments> provideInvalidExpressions() {
        return Stream.of(
                ...
                Arguments.of("activityKind..area.code",
                        List.of("Unknown token 'activityKind..area.code' at position 0")),
                ...
                Arguments.of("substring(activityKind.description 1 10)",
                        List.of("SUBSTRING function requires 'FROM' keyword")),
